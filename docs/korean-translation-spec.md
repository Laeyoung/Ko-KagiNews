...
경로: `data/translations/<batchId>/<categoryUuid>.json` (git 추적 제외 — `.gitignore`에 `data/translations/*` 추가, `.gitkeep`만 커밋). 같은 디렉토리에 `chaos.json`(§4.3의 4단계)과 최상위 `index.json`(북키핑)이 함께 놓인다. 배치는 불변이므로 사이드카도 **완성 후** 불변이다 — 단, 부분 실패 사이드카는 캐치업 크론이 갱신할 수 있으므로 서버 캐시는 §5.1의 무효화 규칙을 따른다.
...
4. **Chaos Index 설명 번역**: `GET /batches/{batchId}/chaos?lang=en` → `chaosDescription`(배치당 1개의 긴 영어 텍스트, 프론트엔드 `chaosIndexService.ts`가 `lang` 파라미터로 요청)을 1회 호출로 번역 → `data/translations/<batchId>/chaos.json` 사이드카 저장. 배치당 1호출이라 비용은 무시 가능.
...
**캐시 무효화 규칙 (중요)**: 사이드카는 "완성 후 불변"이지만, **부분 실패 사이드카는 16:00 UTC 캐치업 크론이 갱신할 수 있다**. 따라서 메모리 캐시는:

- `stats.translated === stats.storyCount`인 **완성 사이드카만 무기한 캐시**한다.
- 미완성 사이드카는 캐시하지 않거나 짧은 TTL(예: 5분)로 캐시한다. (대안: 캐시 엔트리에 파일 mtime을 저장하고 히트 시 `stat`으로 재검증)
...
- `src/routes/api/batches/[batchId]/chaos/+server.ts`, `src/routes/api/batches/latest/chaos/+server.ts` — `chaosDescription`을 chaos 사이드카로 동일하게 오버레이 (§4.3의 4단계).
...
참고 1: `when_not_default` 저장 전략 때문에 사용자가 명시적으로 "한국어"를 선택해도 localStorage 키가 안 생기지만(이미 기본값), 다른 언어 선택은 정상 저장된다.
...
| **P5. 기본값 전환** | §7의 6개 지점(app.html 포함) + contentFilters `ko` 키워드(§8.4) | 신규 방문(localStorage 없음) 시 UI·콘텐츠 모두 한국어, 필터 정상 동작 |
